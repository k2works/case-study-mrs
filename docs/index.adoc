...
component [認証・認可コンテキスト]
component [予約コンテキスト]

[認証・認可コンテキスト]<-->[予約コンテキスト] : Shared Kernel
----
...
include::../cypress/integration/Auth/User.feature[]
----
...
package mrs.domain.model.reservation.reservation {
	package "'Reservation' Aggregate" <<Rectangle>> {
		...
		class ReserbableRoomList <<(V,DarkSeaGreen) Value Object>> {
		}
	}
    Reservation *- ReservationId
    ReservedTime -* Reservation
    Reservation *-- ReservableRoom
    ReservableRoomId -* ReservableRoom
    ReservableRoomId *-- RoomId
    ReservedDate -* ReservableRoomId
    ReservableRoom *- MeetingRoom
    MeetingRoom *-- RoomId
    ReservableRoom --* ReserbableRoomList
}
----
