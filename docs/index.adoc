...
近年、所有物件の稼働率を向上するべく貸会議室運営事業に新たに取り組んでいるが状況は芳しくない。

現在、貸会議室運事業の業務の大半は手作業で行われている。最近、サービスを紹介するWebページと会議室予約システムの初回リリースが行われたが担当者の急な退職により十分な引継ぎが行われなかったため全体を把握する人間が不在の状況となっている。

利用可能な会議室の登録は手作業でシステムに登録しているため、登録漏れや登録間違いにより予約した会議室が使えない、利用したい会議室がいつも使えないなど利用者からのクレームが発生しておりサービス品質向上が求めらている。 また、プロジェクタやホワイトボードなど付随する設備の要望が上がっているが十分に対応できていない状況である。

S社としては、効率化によるサービス品質向上を実現し、サブスクリプションサービスの導入による収益の多様化を実現することで貸会議室運営事業を新たな収益の柱に成長させたいと考えている。
...
[AuthContext]<-->[ReservationContext] : Shared Kernel

[AuthContext]<-->[FacilityContext] : Shared Kernel

[AuthContext]<-->[ContactContext] : Shared Kernel
...
[source,yml]
----
include::../cypress/integration/UserManagement/UserAuth.feature[]

include::../cypress/integration/UserManagement/UserMnagement.feature[]
----
...
[source,yml]
----
include::../cypress/integration/ReservationManagement/RoomSearch.feature[]

include::../cypress/integration/ReservationManagement/RoomReserve.feature[]
...
[source,yml]
----
include::../cypress/integration/ReservationManagement/RoomRegist.feature[]
----
...
[source,yml]
----
include::../cypress/integration/UserManagement/ContactRegist.feature[]

include::../cypress/integration/UserManagement/ContactGustRegist.feature[]

include::../cypress/integration/UserManagement/ContactManagement.feature[]
----
...
image::images/schemaspy/auth/diagrams/summary/relationships.real.large.png[]
